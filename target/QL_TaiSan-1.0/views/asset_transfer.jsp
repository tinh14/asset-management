                ...
                <div class="form-wrapper">
                    <div class="row p-3 border-bottom" id="control-div">
                        ...
                        <div class="col-9 col-sm-9 col-md-4 col-lg-6 col-xl-6">
                            <h5 class="text-primary">CHUYỂN TÀI SẢN</h5>
                        </div>
                        ...
                        <table class="table table-hover">
                            <thead class="sticky-top">
                                <tr class="text-primary">
                                    <th class="col-2 text-center">Mã chuyển tài sản</th>
                                    <th>Ngày chuyển</th>
                                    <th>Người chuyển</div>
                                    <th>Phòng ban nhận</div>
                                </tr>
                            </thead>
                            <tbody>
                                <c:if test="${fn:length(assetTransferList) == 0}">
                                ...
                                <c:forEach var="assetTransfer" items="${assetTransferList}">
                                    <tr>
                                        <td class="text-center">${assetTransfer.id}</td>
                                        <td class="dateIndex"><fmt:formatDate value="${assetTransfer.transferDate}" pattern="dd/MM/yyyy"/></td>
                                        <td>${assetTransfer.transferor.id} - ${assetTransfer.transferor.getFullName()}</td>
                                        <td class="index">${assetTransfer.receivingDepartment.name}</td>
                                    </tr> 
                                </c:forEach>
                            </tbody>
                        </table>
                    ...
                    <div class="row mt-2 justify-content-end">
                        ...
                        <div class="col-sm-6 text-secondary text-right mr-3">
                            Tổng số <span id="numRecords">${fn:length(assetTransferList)}</span> bản ghi
                        </div>
                    </div>
                </div>
            </div>
