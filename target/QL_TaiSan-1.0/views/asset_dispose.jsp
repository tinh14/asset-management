                ...
                <div class="form-wrapper">
                    <div class="row p-3 border-bottom" id="control-div">
                        ...
                        <div class="col-9 col-sm-9 col-md-4 col-lg-6 col-xl-6">
                            <h5 class="text-primary">CHUYỂN HỦY</h5>
                        </div>
                        ...
                        <table class="table table-hover">
                            <thead class="sticky-top">
                                <tr class="text-primary">
                                    <th class="col-2 text-center">Mã chuyển hủy</th>
                                    <th>Ngày hủy</th>
                                    <th>Người hủy</div>
                                    <th>Phòng ban hủy</div>
                                </tr>
                            </thead>
                            <tbody>
                                <c:if test="${fn:length(assetDisposeList) == 0}">
                                ...
                                <c:forEach var="assetDispose" items="${assetDisposeList}">
                                    <tr>
                                        <td class="text-center">${assetDispose.id}</td>
                                        <td class="dateIndex"><fmt:formatDate value="${assetDispose.disposalDate}" pattern="dd/MM/yyyy"/></td>
                                        <td>${assetDispose.disposer.getId()} - ${assetDispose.disposer.getFullName()}</td>
                                        <td class="index">${assetDispose.disposalDepartment.name}</td>
                                    </tr> 
                                </c:forEach>
                            </tbody>
                        </table>
                    ...
                    <div class="row mt-2 justify-content-end">
                        ...
                        <div class="col-sm-6 text-secondary text-right mr-3">
                            Tổng số <span id="numRecords">${fn:length(assetDisposeList)}</span> bản ghi
                        </div>
                    </div>
                </div>
            </div>
